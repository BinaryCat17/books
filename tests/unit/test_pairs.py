...
def _each_model_box_once(got, m):
    """The three disjoint sets `compare_pages` describes cover the model's
    boxes: pass A's finds, pass B's finds of text boxes, and the extras."""
    a_finds = [e["run"] for e in got["pairs"] if e["by"] == "A" and e["run"]]
    b_finds = [e["b_run"] for e in got["pairs"]
               if e["b_run"] and policy.UNION.role(
                   next(x["label"] for x in m["blocks"]
...
def _one_page(slovar, want_artefact=True):
    T = truth_pages(slovar.truth_dir)
    i, t = next((i, t) for i, t in T.items()
...
def test_a_box_pass_b_took_for_an_artefact_pass_a_missed_is_in_the_list_once(slovar):
    """Pass A misses the table (eaten by a text box laid exactly on it) and
    pass B pairs the two: the entry is missed by A, names B's partner and
    its label disagreement, and the eater is nowhere else -- neither an
    extra nor gone. The old list dropped it; the sheet drew nothing."""
...
def test_an_artefact_box_pass_a_left_and_pass_b_took_is_one_extra_that_says_so(slovar):
    """A model artefact box lying exactly on a truth text block: pass A
    calls it spurious, pass B pairs it with the text block. The list keeps
    both facts as the count does -- one extra naming the block B took it
    for, one matched text entry naming it -- and the box is one extra, not
    an extra and a find."""
    i, t, m = _one_page(slovar)
    txt = next(b for b in t["blocks"] if policy.UNION.role(b["label"]) == "text")
    arte_label = next(b["label"] for b in t["blocks"]
...
def test_label_ok_is_the_drawn_pairs_and_the_label_metrics_pair_rides_beside(slovar):
    """Truth table; the model gives a loose table box and a text box laid
    exactly on the table. Pass A pairs the table with the table box, label
    agreeing; pass B, blind to labels, takes the exact text box. The entry
    says both, and the label metric names the box B took."""
    i, t, m = _one_page(slovar)
    art = next(b for b in t["blocks"] if policy.UNION.role(b["label"]) == "artifact")
    text_label = next(lb for lb in policy.UNION.labels if policy.UNION.role(lb) == "text")
    x0, y0, x1, y1 = art["box"]
    ...
    for b in m["blocks"]:
        if b["block_id"] == art["block_id"]:
            b["label"] = text_label
    m["blocks"].append({"block_id": loose_id, "box": [x0 - 6, y0 - 6, x1 + 6, y1 + 6],
                        "label": art["label"], "score": 0.9, "order": None,
...
def test_a_page_that_says_it_is_not_labelled_is_left_out_and_said(slovar):
    ...
    # out; asked alone it must not answer as if it were compared.
