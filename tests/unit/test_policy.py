...
`policy.py` is complete by construction, and the price of a default is known: a
threshold dictionary with one class silently gave the rest 0.5, and a new class
of weights would spill into the prose as one artefact fewer.
...
def test_check_does_not_use_the_union():
    """The check is against the named dictionary, not the union of all five: the
    union covers foreign labels too, and `Table` from DocLayNet in a V2 run means
    swapped weights, not a lawful label."""
    try:
...
def test_union_agrees_with_every_dictionary():
    """`ROLE` is the union with a contradiction check: one label cannot mean
    different things in two dictionaries, and a role cannot depend on import
    order."""
    for name, table in policy.POLICIES.items():
        for lab, r in table.items():
...
def test_adapters_and_policies_agree():
    """An agreement through a file: adapter dictionary -> policy name. `detect.py`
    calls `policy.check` with the model's dictionary, so a divergence here drops
    the adapter's run on its own bench."""
