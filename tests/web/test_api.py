...
cancel that stops it, a dead process that leaves a failed row, and a key
...
def _upload(client, pdf, name):
    with open(pdf, "rb") as f:
        r = client.post("/api/books", files={"file": (os.path.basename(pdf), f, "application/pdf")},
                        params={"name": name})
...
def test_each_user_sees_only_their_own_books(app, slovar):
    ...
    # The same scan twice in one store is refused: two books of one scan.
...
def test_a_detect_job_runs_reports_its_progress_and_files_its_run(app, home, slovar, served_endpoint):
    ...
    # A job is its owner's: another user gets nothing, the admin everything.
    bob = as_user(app, "bob")
...
def test_a_cancel_stops_a_running_job_between_pages(app, home, slovar):
    def slow(page):
        time.sleep(0.4)
        return page

    with FakeLayout(slovar.truth_dir, answer=slow) as fake:
        _registry(home, {"slow": {"kind": "layout", "endpoint": fake.url, "knobs": {}}})
        alice = as_user(app, "alice")
        book = _upload(alice, slovar.pdf, "slovar")
        job_id = alice.post("/api/jobs", json={"kind": "detect", "book": book,
                                               "model": "slow"}).json()["id"]
        for _ in range(100):
        ...
        assert fake.requests < 13, "the job ran to the end after the cancel"
    store = app.state.settings.store_of(app.state.db.user("alice")["id"], "user")
    run = os.path.join(store, "processed", "slovar", "detect", "truth")
...
def test_a_users_read_job_carries_the_entrys_key(app, home, slovar, served_endpoint):
    with FakeVlm({"text": "read", "finish": "stop"}) as vlm:
        _registry(home, {
            ...
            "vl": {"kind": "reader", "endpoint": vlm.url, "api_key": "sk-entry",
...
def test_the_registry_is_written_checked_and_users_are_the_admins(app, home):
    ...
    good = {"m": {"kind": "reader", "endpoint": "http://h/v1", "knobs": {"PAGE_DPI": 72},
                  "api_key": "sk"}}
