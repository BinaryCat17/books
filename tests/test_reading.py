...
def test_the_battery_can_fail_on_pages_that_carry_answers():
    ...
    pages = _pages([("chart", CHART), ("text", "recognised words here"),
                    ("table", "a | b\n1 | 2")])
    assert reading.mutations(pages, log=lines.append) == 0, lines
    tail = lines[-1]
    assert "probes 6" in tail and "nothing to measure with 0" in tail, tail
    assert not any("no data" in l for l in lines[:-1]), lines
