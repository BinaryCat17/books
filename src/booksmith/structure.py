...
def relabel(text, blks):
    ...
    while i < len(paras):
        ...
        if chapter:
            # Название главы бывает разорвано на два блока.  У главы 29 этой
            # книги оно приехало целиком (`PRECISION GRINDING MACHINES\nTHE
            # CYLINDRICAL GRINDER`), а у главы 30 — двумя блоками подряд, и
            # вторая половина оставалась отдельным заголовком уровня заглавия
            # книги посреди главы.  Сращиваем: следующий блок берём в название,
            # если он тоже заголовочный, сам главой не является и стоит в
            # книге прямо за нашим абзацем.
            j = b
            while j < len(blks) and j < b + 2:
                lb2, c2 = blks[j]
                if lb2 not in ("header", "doc_title"):
                    break
                t2 = " ".join(x.strip() for x in c2.split("\n") if x.strip())
                if CHAPTER.match(t2) or CHAPTER_FULL.match(t2):
                    break
                q = i + used
                while q < len(paras) and not paras[q].strip():
                    q += 1
                if q < len(paras) and _key(paras[q]) == _key(t2):
                    tail = re.sub(r"^#{1,6}\s+", "", paras[q].strip())
                    orig = orig + " " + " ".join(tail.split())
                    used = q - i + 1
                    b = j + 1
                    counts["хвост названия главы прирос"] += 1
                break
...
def _measure(text):
    ...
    hs = _headings(body)
    numbered = [h for h in hs if h[1] == "sub" and SEC_IN_TITLE.match(h[2])]
    return {
...
def _ranges(text, hs):
    ...
    for n, (i, role, t, num, depth) in enumerate(hs):
        if role == "chapter":
            ...
            if nxt is None:
                # Только верхний уровень: иначе главу обрывает первый же
                # служебный ярлык.  Замер: `### PROCEDURE:` без цифр стоит
                # внутри тридцатой главы, и без ограничения по глубине она
                # кончалась на строке 17812 вместо 19305.
                nxt = next((hs[m][0] for m in range(n + 1, len(hs))
                            if hs[m][4] <= 2
                            and not any(c.isdigit() for c in hs[m][2])), None)
