...
# Номер раздела в начале названия: «Sec. 10.5», «10.5», «§ 3.4».  Приставки
# рисунка и таблицы исключены: подпись `Fig. 26.69 Accuracy of lead screw`
# иначе считается разделом, получает ложную пометку «порядок нарушен» и —
# хуже — сдвигает счётчик, из-за чего следующий честный раздел тоже метится.
# Список приставок, как и список слов «глава», — место, куда дописать своё.
NOT_SECTION = ("fig", "figs", "figure", "рис", "рисунок", "abb", "abbildung",
               "bild", "table", "tab", "табл", "таблица", "plate", "pl",
               "eq", "форм", "формула", "photo", "foto")
...
def _section_number(title):
    ...
    if not m:
        return None
    if m.group(1).strip(".").casefold() in NOT_SECTION:
        return None
...
def toc(text):
    ...
    for i, role, t, num, depth in hs:
        ...
        tail = ""
        if role == "chapter" and ends[i] == n_lines:
...
def restructure(outdir):
    ...
    if not after["глав"] and after["заголовков всего"]:
        # «глав 0 из 0» читается как успех, а это промах словаря: книга,
        # где глава названа «Часть» или «ГЛАВА ПЕРВАЯ» словами, не опознаётся
        # вовсе.  Молчать тут нельзя — рушится и оглавление, и проверка
        # порядка (без номера главы каждый раздел считается вставшим не на
        # своё место).
        print(f"  ВНИМАНИЕ: заголовков {after['заголовков всего']}, а глав "
              f"не опознано ни одной. Слово «глава» в этой книге не из списка "
              f"CHAPTER_WORDS ({', '.join(CHAPTER_WORDS[:3])}, …) — допиши "
              f"его, иначе оглавление и проверка порядка недостоверны.")
