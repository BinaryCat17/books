...
def _looks_heading(text):
    ...
    Цена решения названа честно: `For example:` заголовком остаётся — он начат
    с прописной.  Это косметика.  А `by OBJECTIVE NO. 1. A tolerance of .0004"
    in 6" is permissible.` — числовое утверждение в виде названия раздела —
    отвергается, и ради этого проверка и писалась.
    ...
    while i < len(paras):
        ...
        if k:
            for j in range(b, min(len(blks), b + WINDOW)):
                ...
                # Совпадение по началу абзаца разрешаем длинным ключам и,
                # отдельно, блоку-номеру главы: «Глава 3» — семь знаков, и
                # прежний порог `len(kb) >= 8` отсекал его целиком, из-за чего
                # на русской книге блок оставался непогашенным и его подбирал
                # следующий абзац прозы.
                #
                # Послаблять порог ВСЕМ коротким ключам нельзя, и это проверено
                # прогоном: колонтитулы и номера страниц вычищаются из `.md`,
                # но остаются в `pages/*.json`, поэтому на каждой странице
                # висит непогашенный блок в один-два знака.  С общим
                # послаблением блок «3» (номер страницы) перехватывал заголовок
                # «3.1», и заголовок терялся молча.
                head = kb and CHAPTER.match(blks[j][1].strip())
