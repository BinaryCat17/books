...
class Box:
    ...
    def probe_download(self, mb: int = 12, streams: int = 6,
        ...
        # константу `chunk * streams`: пробный файл весит 20 МБ, а шесть
        # диапазонов покрывают 72 МБ, то есть четыре потока из шести просят
        # за концом файла и получают 416 — который curl ошибкой не считает.
        # Машина с закрытым выходом наружу отвечала мгновенно и без единого
        # байта, формула выдавала ~3000 Мбит/с, число уходило в журнал
        # рекордом, и `fast_machines` ставила такую машину первой. Соседний
        # `probe()` сделан правильно; здесь проверки не было вовсе.
        parts = " ".join(
        ...
        if ns <= 0:
            return 0.0
        want = min(chunk * streams, self.PROBE_FILE_BYTES)
        if got < want * 0.5:
