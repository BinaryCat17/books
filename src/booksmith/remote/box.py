...
`outputs/` is pulled as the work goes, so a crash late in a job loses only the
pages after the last sync.
"""
import os
import re
import select
...
# Own directory at mode 0700: ssh does not fall back when it cannot bind the
# control path (rc=255), and it checks neither owner nor mode of a socket
# already there.  The path is capped at 108 bytes; the worst case here is 47.
...
SSH_OPTS = [
    ...
    # Without this, ssh to a machine gone silent hangs forever.
    "-o", "ConnectTimeout=15",
...
class Box:
    ...
    def __init__(self, user: str, host: str, port: str,
        ...
        # "not checked" and not "": not asked is not the same as not armed.
    ...
    def run(self, cmd: str, stream: bool = True,
        ...
        `deadline` is absolute time past which the command is killed: a job must
        not outlive the money allotted to it.
        """
        full = self._ssh + [self._addr, cmd]
        if not stream:
            if deadline is not None and deadline <= time.time():
                # The term is already gone: no grace, or ssh is killed locally
                # while the remote command keeps working.
        ...
        # select, not iteration over p.stdout: a job silent for minutes (vLLM
        ...
        try:
            ...
            if p.poll() is None:
                # After a normal end ssh is not reaped yet: killing here would
                # turn a successful run into -9.
                try:
                    p.wait(timeout=15)
                except subprocess.TimeoutExpired:
                    p.kill()
    ...
    def probe(self, seconds: float = 12.0, mb_cap: int = 64) -> float:
        ...
        Measured by time and not by size, so there are no false zeros: zero means
        not one byte came.  Data from /dev/urandom -- zeros would compress in ssh
        -- and `mb_cap` only keeps the stream finite.
        """
        # `ControlMaster=no` uses a shared master but never becomes one: the
        # probe kills ssh when its time is up, taking any rider with it.
    ...
    # Probe file size: a range past its end returns 416 and zero bytes.
    ...
    def probe_download(self, streams: int = 6, timeout: float = 40.0) -> float:
        ...
        Several because uv fetches wheels over dozens of connections, and the
        chunk length is derived from the file so that the streams divide it
        exactly.  The number only goes into the ledger; it rejects nothing.
        """
        url = ("https://files.pythonhosted.org/packages/source/n/numpy/"
               "numpy-2.2.0.tar.gz")
        streams = max(1, int(streams))
        chunk = self.PROBE_FILE_BYTES // streams
        # -L is mandatory: pythonhosted answers 302, and without it curl fetches
        # zero bytes in silence and the machine looks broken.
        # The bytes counted are the ones that arrived, not the ones ordered: a
        # range past the end returns 416, which curl does not call an error.
        # The last chunk takes the remainder, so the file is covered once.
        parts = " ".join(
        ...
        if got < want * 0.5:
            # Less than half of what was ordered arrived: this zero says the
            # probe did not work, not that the channel is fast.
            return 0.0
        return got * 8000.0 / ns

# Ceiling on a single transfer: 180 MB over a 2.9 Mbit/s link is sixteen minutes,
# and without a ceiling a stuck rsync holds the sync thread forever.
    ...
    def weigh_exclude(self, remote_rel: str, exclude: tuple, local_dir: str):
        ...
        Measured by rsync itself, `--dry-run --stats` with the exclusions and
        without: rsync reads a pattern its own way, so any other reading weighs
        something else.  Returns `(pattern, bytes, files)` rows.
        """
    ...
    def push(self, local: str, remote_rel: str) -> None:
        ...
        rsync lands a source without a trailing slash inside the destination, so
        a directory gets a slash on both ends; scp ignores that slash, and only a
        `/.` source puts the contents in place whether the destination exists.
        """
        ...
        if os.path.isdir(local):
            src = local.rstrip("/") + "/"
            dst = dst.rstrip("/") + "/"
        # rsync creates only the last link of the path, so a nested name needs
        # its parent made first.
    ...
    def pull(self, remote_rel: str, local_dir: str, quiet: bool = False,
        ...
        Needed when the result directory is mostly what we do not want: 167 MB of
        179 were images on one run, sixteen minutes of transfer for nothing.
        """
    ...
    def start_heartbeat(self, every: float = 30) -> None:
        ...
        The onstart deadman watches this file and destroys the instance once it
        goes stale.  Touch errors are swallowed: the term is minutes, so one
        missed attempt decides nothing.
        """
        def loop():
    ...
    def stop_heartbeat(self) -> None:
        ...
        Otherwise our own thread keeps feeding the deadman of a machine we have
        given up on, and it never kills itself.
        """
        self._stop_hb.set()
...
# Ceiling on a short service command: ample for `echo`+`mv` over an open
# connection, and a silent machine then cannot freeze the `finally` path.
    SHORT_CMD_S = 60.0

    def check_deadman(self, tries: int = 3, pause: float = 4.0) -> str:
        """Is the deadman armed on the machine itself.  Silence is not allowed.

        The only kill path depending neither on our process nor on our key.
        Three tries because onstart writes the report in the background and the
        order against ssh coming up cannot be relied on.
        """
        # One declaration of the path, beside the ONSTART that writes the file.
        # Imported here so `box.py` does not pull `vastai` for one line.
        ...
        for i in range(max(1, tries)):
            ...
            state = (out or "").strip()
            # `ARMED` in latin: the report crosses the vast API, a remote shell
            # and ssh, and UTF-8 over that path cannot be checked without renting.
    ...
    def set_deadman(self, seconds: int) -> None:
        ...
        Temp file and atomic move: a broken `echo > file` leaves it empty, and an
        empty term expands to `[ N -gt ]`, a syntax error, so the machine never
        kills itself.  The return code is examined, not swallowed.
        """
        rc, out = self.run(
    ...
    def stop_sync(self) -> None:
        ...
        Otherwise two rsyncs write into one directory at once.
        """
        self._stop_sync.set()
