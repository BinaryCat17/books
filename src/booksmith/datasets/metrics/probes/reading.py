...
from booksmith.datasets.metrics import reading as m
...
def probes(bench, run) -> list:
    ...
    return [Probe(n, w, f) for n, w, f in (
        ...
        # THE RULE IS THE LABEL AND THE SHAPE TOGETHER. The shape alone would
        # call every real table a fabrication; the label alone would call a
        # chart described in words one.
        ...
        # A declared merge is not a loop, which is why the markup goes first.
