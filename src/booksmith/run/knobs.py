...
KNOBS = (
    ...
    Knob("HTML_MATH", "local",
         "чем рисовать формулы в книге: local (MathJax рядом с ней, 2.11 МБ, "
         "сеть не нужна) | cdn (тянуть из сети при каждом открытии) | off "
         "(сырой LaTeX). Умолчание `local`, потому что книгу кладут на диск и "
         "читают через полгода. Замер, ради которого ручка заведена: в книге "
