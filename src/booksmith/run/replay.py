...
ЧЕТЫРЕ БЕДЫ, А НЕ ОДНА, И СТРОКИ У НИХ РАЗНЫЕ.  Величины НЕТ (пропуск, код
...
величину НЕ ПОКРЫТЬ ВОВСЕ.  Числа у них разные, и в выводе они разными
строками.
...
def selfcheck(outdir, log=print) -> int:
    ...
    ПЯТЬ БЕД, ПЯТЬ ЧИСЕЛ, ОДНА СУММА В ВОЗВРАТЕ. Не пойманная пропажа
    ...
    снят другим адаптером), и неопознанный писатель (сверять нечем вовсе) —
    беды разные, и в журнале они разными величинами. Складываются они только
    на выходе, потому что молчат одинаково: без кода возврата любая из них
    остаётся заметкой, которую никто не читает. Ноль в возврате —
    «спрошено и не найдено», а не «не спрашивали»: когда `run.json` не
    читается, возвращается len(req), а когда форму отпечатка не с чем
    сверить — число невыведенных величин, а не ноль.

    ШЕСТОЕ ЧИСЛО ПЕЧАТАЕТСЯ, НО В СУММУ НЕ ВХОДИТ, и это решение, а не
