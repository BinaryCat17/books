...
def mutations(truth_dir: str, pages_dir: str, log=print) -> int:
    ...
    # ПРОБА ПРИМЕНИМА НЕ ВСЕГДА, и это надо сказать, а не провалить. Она бьёт
    # по ступени ГЕОМЕТРИИ, а на входе, где всё сведено по якорю, геометрия не
    # решает ничего — сдвиг рамок обязан не менять НИЧЕГО, и рядом стоит
    # проба, которая именно этого и требует («с якорями рамки сдвинуты на 0.9:
    # сопоставление держится»). Прежде тут печаталось «НЕ ПОЙМАНА» на
    # безупречном входе: батарея обвиняла метрику в том, что та ведёт себя
    # правильно.
    def shifted():
        m = base["сопоставление"]
        if m["по якорю"] and not (m["по номеру"] + m["по геометрии"]):
            return None
