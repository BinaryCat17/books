...
def mutations(truth_dir: str, pages_dir: str, log=print) -> int:
    ...
    def anchor_gate():
        m = M(_shift_boxes(_anchor_all(P)))["сопоставление"]
        return (fell(m["доля"], b_match)
                and grew(m["якорь мимо рамки"], b_offbox))

    probes.append(("с якорями рамки сдвинуты на 0.9",
                   "сопоставление падает, «якорь мимо рамки» растёт",
                   anchor_gate))
