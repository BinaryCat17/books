...
def compare_pages(T: dict, M: dict) -> dict:
    ...
    for i, t in sorted(T.items()):
        ...
        for j, x in enumerate(mb):
            if j in used:
                continue
            if any(cover(x["box"], b) >= 0.9 for b in paired):
                bed("вложенный дубль")
            elif any(cover(x["box"], b) >= 0.9 for b in unpaired):
                bed("внутри ненайденного")
            elif (any(cover(x["box"], b) >= 0.5 or cover(b, x["box"]) >= 0.5
                      for b in outside)
                  # …но НЕ тогда, когда та же рамка накрывает ещё и артефакты
                  # истины: рамка во весь лист пересекается с любой буквицей,
                  # и амнистия прощала бы ей заодно съеденные таблицы.
                  and sum(1 for b in tb
                          if cover(b["box"], x["box"]) >= 0.6) < 2):
                bed("на объекте вне замера")
            else:
                bed("лишняя рамка")
...
def column_jumps_ranking(variants: dict, grid: dict = None,
    ...
    for p in pts:
        for n in names:
            vals[n].append(column_jumps(variants[n], **p)[key])
    flips, ties = [], []
...
def _columns_of(p):
    ...
    part, rest = [], []
    for b in p["blocks"]:
        if (policy.role(b["label"]) in COLUMN_ROLES
                and (w <= 0 or (b["box"][2] - b["box"][0]) < COLUMN_WIDE * w)):
            part.append(b)
        else:
            rest.append(b)
    return part, rest
...
def _mix_columns(M):
    ...
    for i, p in M.items():
        ...
        buckets = {}
        for c, b in zip(_columns([b["box"] for b in part]), part):
...
def _by_reading(M):
    """Наше правило сборки: сверху вниз, слева направо. Тот самый порядок,
    который адаптеры объявляют словом «наш»."""
    return {i: {**p, "blocks": sorted(
        p["blocks"], key=lambda b: (b["box"][1], b["box"][0]))}
        for i, p in M.items()}
...
def _by_columns(M):
    ...
    for i, p in M.items():
        part, rest = _columns_of(p)
        col = _columns([b["box"] for b in part])
...
def _order_variants(M):
    """Четыре порядка сборки НА ОДНИХ И ТЕХ ЖЕ рамках.
    ...
    return {"как отдала модель": M,
            "сверху вниз, слева направо": _by_reading(M),
            "колонка за колонкой": _by_columns(M),
            "по кругу через колонки": _mix_columns(M)}
