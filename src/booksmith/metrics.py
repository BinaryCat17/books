...
def _by_reading(M, overlap=None, wide=None, min_boxes=None, roles=None):
    ...
    Параметры группировки здесь не участвуют — правило чисто геометрическое,
    колонок оно не знает, — и принимаются они только затем, чтобы точка
    развёртки передавалась всем сборщикам одинаково."""
