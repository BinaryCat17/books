...
`docs/ocr-notes.md` до сих пор предписывает сверять числа с собственным
текстовым слоем PDF, а код извлечения (`engines/pdf_layer.py`, 375 строк)
удалён. Слой был единственным свидетелем, который **не Mistral и не стоит
денег**.
