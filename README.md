...
│   ├── models/   адаптеры: base.py — контракт, doclayout.py — детектор
│   │              контуров (ONNX, процессор), paddleocr_vl/ — доставка VLM
│   ├── books/    книги синтетического стенда: словарь, атлас, каталог,
│   │              учебник математики, сборник статей
│   ├── doc/      из контуров в HTML: crop, feed, html, swap
│   ├── run/      реестр ручек, слепок входа, replay
│   ├── detect.py первый уровень: контуры страниц, местно и бесплатно
│   ├── synth.py  синтетический стенд: истина меряется по чернилам
│   ├── metrics.py метрики контуров и батарея мутаций
│   ├── overlay.py рамки поверх страниц — посмотреть глазами
│   ├── policy.py ярлык -> разряд: артефакт | текст | служебное
│   ├── djvu.py   djvu -> PDF с разрезом разворотов
│   └── cli.py    books <команда>
├── infra/base/   Dockerfile образа-прихожей (сборка в GitHub Actions)
├── bench/        синтетические книги стенда и страницы будущего золотого
│                 стенда (см. bench/README.md)
...
Порядок работ: стенд и метрики **до** моделей. Стенд и метрика построены;
первый уровень работает целиком на процессоре и бесплатно. Что им померено —
`docs/contour-notes.md`. VLM второго уровня пока не подключена.
...
books replay --selfcheck …   умеет ли САМА проверка провалиться
