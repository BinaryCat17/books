...
books convert processed/книга             HTML, EPUB, FB2 рядом с текстом
books merge processed/книга               пересобрать книгу из проходов, без GPU
books restructure processed/книга         пересобрать заголовки и отчёт, без GPU
books tidy processed/книга                перевести старый разбор в новую раскладку
...
книгу на перевод или пересказ, написано в `work_instruction.md`. Агенту,
запущенному из этого каталога, его укажет `CLAUDE.md`; в обычный чат файл
вставляется целиком как готовый промт. Правила в нём общие для любого скана и
ничего не знают о конкретной книге — что это за издание и чем опасна ошибка,
скажите модели сами. Замеры по нашей книге — в `docs/ocr-notes.md`.
...
Там же добавки отдельно под перевод и под пересказ.

`--passes 3` кладёт проходы в `выход/passes/1..3` и собирает книгу наверху.
Второй и третий идут на той же машине, так что холодный старт платится один
раз; проход по книге в 539 страниц занимает около пяти минут счёта.

Каталог разбора выглядит так — наверху только готовое, всё черновое в
`passes/`:

```
processed/книга/
  <книга>.md  <книга>.toc.md  <книга>.html .epub .fb2
  report.md   run.json   imgs/
  passes/1/ 2/ 3/
```

`<книга>` — имя исходного pdf: каталог оператор называет наспех, а книга должна
называться книгой. Точное имя записано в `run.json` полем `stem`.

Всё, что можно посчитать местно, считается местно — `merge`, `restructure`,
`convert` карту не арендуют. Правку, которую нельзя применить к уже оплаченному
разбору, применяют к нему никогда.
