...
за восемь аренд ($0.545 по `runs/ledger.jsonl`, две удачных), а `books swap
...
books swap выход/            что заменено; --anchor … --file … — заменить,
                             --from чтение/ — поставить ВСЁ прочитанное разом,
                             --undo — откатить. Сама модель тут не зовётся:
                             её позвал `books read` раньше
...
- `imgs/` подряд не читай: на них тысячи ссылок из текста, они нужны тексту, а
  не чтению подряд. (Здесь стояло «2266 ссылок» — число не воспроизводится ни
  одним способом счёта, ближайшее 4450, и потому убрано, а не поправлено.)
