...
ALLOWED = ('manifest.json', '<source>.pdf', 'truth/', 'detect/<model>/', 'read/<model>/', 'look/<model>.pdf', 'look/truth.pdf', 'assets/', 'book.html')
BESIDE_A_RUN = ('.crop', '.read')
INSIDE_A_RUN = {'detect': ('pages', 'run.json'), 'read': ('pages', 'answers', 'crops', 'html', 'run.json', 'read_with.json', 'job.log', 'job', 'vllm.json', 'vllm.log')}
ROOT_FILES = ()

def safe_label(name: str, what: str) -> str:
    name = (name or '').strip()
    if not LABEL_OK.match(name):
        raise Refusal(f"{what}: {name!r} cannot be a run directory name. A label is the MODEL's name, letters, digits and . _ + - only. This one comes from the model itself (the weights' own name, the variant, the weights file), so a name like this means the weights do not declare one -- pass --run <name> and the run is filed under that.")
    return name
KINDS = ('detect', 'read')
...
class Book:
    ...
    def open(cls, path: str, what: str='book') -> Book:
        ...
        if not os.path.isfile(man):
            raise Refusal(f'{what}: {path} is not a book directory -- no manifest.json. A book directory is made by the first command that writes into it, and the manifest is what says WHICH PDF it is about; without it a page set can be measured against a book nobody named.')
        import json
    ...
    def runs(self, kind: str) -> builtins.list[str]:
        ...
        if not os.path.isdir(base):
            return []
        return sorted(n for n in os.listdir(base) if os.path.isfile(os.path.join(base, n, 'run.json')) and os.path.isdir(os.path.join(base, n, 'pages')))

    def one_run(self, kind: str, label: str='') -> str:
        if label:
            d = self.run_dir(kind, label)
            if not os.path.isdir(d):
                raise Refusal(f'{self.name}: no {kind} run labelled {label!r}. There is {self._listing(kind)}')
            return d
        got = self.runs(kind)
        if len(got) == 1:
            return self.run_dir(kind, got[0])
        if not got:
            raise Refusal(f'{self.name} has no {kind} run at all. Make one first: ' + (f'`books detect {self.root}`' if kind == 'detect' else '`books read <detect dir>` -- level two does not write into the book directory yet, see the header'))
        raise Refusal(f"""{self.name} has {len(got)} {kind} runs and none was named: {', '.join(got)}. Say --run <label>; measuring "the" run when there are several would file the number against whichever sorted first.""")
...
def guard_identity(run_dir: str, identity: str, pages_spec: str='', what: str='this run') -> None:
    ...
    try:
        ...
        raise Refusal(f'{snap} cannot be read ({type(e).__name__}), and it is what says whether {what} is the same experiment as the one already there. Remove the directory or give --run a new label.')
    old = was.get('identity')
    if old is None:
        raise Refusal(f'{run_dir} holds a run whose snapshot records no identity -- it predates them -- so there is no telling whether {what} is the same experiment. Give --run a new label, or remove it.')
    if old != identity:
        raise Refusal(f"{run_dir} holds a DIFFERENT experiment: identity {old[:12]} against {identity[:12]}. Same model, other conditions -- a knob the run reads, or other weights. Writing here would leave one snapshot over two runs' pages. Give --run a new label and the two stand side by side, which is what labels are for.")
    if pages_spec:
        raise Refusal(f'{run_dir} already holds a run, and --pages {pages_spec} would write a PART of one over it. The identity cannot see a page selector -- it is over the model and the knobs -- so the snapshot would describe a run that never happened over pages left from the one that did. Give --run a new label, or remove the directory: its two sibling refusals say both and this one said only the first, which made re-running a three-page smoke test look forbidden.')
...
def page_files(d: str) -> tuple[int, str]:
    ...
    if not (isinstance(first, dict) and 'blocks' in first and ('index' in first)):
        return (0, f'json files {len(names)}, but {names[0]} is not a layout page: no blocks/index fields')
    return (len(names), '')

def pages_dir(path: str, what: str) -> str:
    if not os.path.exists(path):
        raise Refusal(f'{what}: no path {path}. Expected a `books detect` run directory (pages/ and run.json in it) or the directory of layout pages itself (*.json).')
    ...
    if here:
        return path
    raise Refusal(f'{what}: no layout pages found. In {path} — {why_here}; in {sub} — {why_sub}. Expected a `books detect` run directory (pages/ and run.json in it) or the page directory itself. There is nothing to count — and that is not a zero of losses.')

def run_dir(path: str, what: str) -> str:
    if not os.path.exists(path):
        raise Refusal(f'{what}: no path {path}. Expected a `books detect` run directory — the one holding run.json.')
    ...
    if page_files(path)[0] and os.path.exists(os.path.join(up, 'run.json')):
        ...
        return up
    raise Refusal(f'{what}: no run.json in {path}. Expected a `books detect` run directory (pages/ and run.json in it), not a page directory and not a book root.')
