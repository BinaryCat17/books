...
per book; for admins a label mode that starts a truth, draws and relabels
its boxes, orders them and saves a layer), and an admin panel (the
registry, users, placements, the ledger).
...
mapping, the verdict logic, the labeler's edits). The image builds the app and serves it
...
label a page, the admin panel) and screenshots each screen; it needs the `playwright`
