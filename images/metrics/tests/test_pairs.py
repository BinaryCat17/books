...
def test_truth_against_itself_pairs_every_block_and_the_record_keeps_none_of_it(bench):
    T = page_mod.load_pages(bench.truth_dir)
    M = _model_of(T)
    ...
    rec = contour.ContourMetric().record(res, "bench", "truth")
...
def test_a_lost_block_an_extra_box_and_a_renamed_one_are_said_as_the_count_says(bench):
    T = page_mod.load_pages(bench.truth_dir)
    M = _model_of(T)
    i, art, txt = next(
        (
...
def _one_page(bench, want_artefact=True):
    T = page_mod.load_pages(bench.truth_dir)
    i, t = next(
        (
            (i, t)
...
def test_a_box_pass_b_took_for_an_artefact_pass_a_missed_is_in_the_list_once(bench):
...
def test_label_ok_is_the_drawn_pairs_and_the_label_metrics_pair_rides_beside(bench):
    i, t, m = _one_page(bench)
    art = next(b for b in t["blocks"] if policy.UNION.role(b["label"]) == "artifact")
    text_label = next(lb for lb in policy.UNION.labels if policy.UNION.role(lb) == "text")
    x0, y0, x1, y1 = art["box"]
...
def test_two_rasters_refuse_in_page_pairs_as_in_compare(bench):
    i, t, m = _one_page(bench)
...
def test_a_page_that_says_it_is_not_labelled_is_left_out_and_said(bench):
    T = page_mod.load_pages(bench.truth_dir)
    M = _model_of(T)
