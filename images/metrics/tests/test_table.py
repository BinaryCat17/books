...
from metrics.base import Record, Scalar
...
def test_a_book_without_truth_is_measured_by_what_needs_none():
    with tempfile.TemporaryDirectory() as d:
        ...
        assert "text: NOT MEASURED, this book and run give no content, read, truth" in text, text
        from metrics import metrics as registry
        from metrics.base import applicable
...
def test_two_levels_of_one_model_do_not_land_on_one_results_file():
    with tempfile.TemporaryDirectory() as d:
        ...
        a, z = (
            table.results_path(Bench.open(root), det),
...
def test_the_report_leaves_out_a_run_of_another_level_and_counts_it():
    ...
    with tempfile.TemporaryDirectory() as d:
        table.write_json([red], os.path.join(d, "b-read-SomeReader.json"), kind="read")
        ...
        assert [(k, n, b, r) for k, n, b, r, _ in other] == [
            ("read", "b-read-SomeReader.json", "bench/b", "SomeReader")
        ], other
...
def test_an_undefined_jump_count_says_why_instead_of_printing_zero():
    ...
    for name in ("excess_jumps", "excess_jumps_per_page"):
        ...
        assert sc.why and "UNDEFINED" in sc.why, (
            f"{name} is absent and does not say why: {sc.why!r}. A zero here would read as 'no excess jumps', which is another thing"
        )
...
def test_the_report_refuses_a_record_of_a_run_that_is_gone_and_counts_the_rest():
    ...
    rec = Record(
        "fitness", "b", LABEL, {"ink_under_boxes": Scalar(0.5)}, identity="one", book="bench/b"
    )
    with tempfile.TemporaryDirectory() as d:
        ...
        with support.said():
            table.write_json(
                [Record("fitness", "b", LABEL, {"ink_under_boxes": Scalar(0.5)})],
...
def test_a_bench_and_a_processed_book_of_one_name_are_two_cells_each_checked_against_its_own_run():
    ...
    with tempfile.TemporaryDirectory() as d:
        for root, ident in (("bench", "one"), ("processed", "two")):
            ...
            with open(os.path.join(run_dir, "run.json"), "w", encoding="utf-8") as f:
                json.dump(
                    {"identity": ident, **({"layout": "own"} if root == "processed" else {})}, f
                )
            rec = Record(
                "fitness",
                "x",
                "R",
                {"ink_under_boxes": Scalar(0.5)},
                identity=ident,
                book=f"{root}/x",
            )
