...
from metrics.bench import Bench
from metrics.base import Metric, Probe, Record, Scalar, applicable, run_probes
...
def _slovar():
    if not (
        os.path.isdir(os.path.join(SLOVAR, "truth"))
...
def test_records_agree_with_the_raw_dict_they_carry():
    ...
    for m in registry.METRICS:
        ...
        for k, s in rec.scalars.items():
            ...
            if s.value is not None:
                ...
                if s.count is not None:
                    n, of = s.count
                    assert of and abs(s.value - n / of) < 1e-09, (m.name, k, s.value, s.count)
            else:
        ...
        assert Record.from_json(json.loads(json.dumps(rec.to_json()))).scalars == rec.scalars, (
            m.name
        )
    contour = registry.BY_NAME["contour"].run(b, r)
    d = contour.detail
    assert contour.scalars["artefacts_found"].value == d["totals"]["share"]
    assert contour.scalars["artefacts_found"].count == (
        d["totals"]["found"],
        d["totals"]["artifacts"],
    )
...
def test_applicability_is_by_prerequisite_not_by_trait():
    ...
    assert names(Full()) == ["assembly", "contour", "fitness", "reading", "snapshot", "text"]
...
def test_a_reading_metric_is_not_applicable_to_a_run_that_read_nothing():
    ...
    names = {m.name for m in base.applicable(registry.METRICS, b, run, truth, empty)}
    ...
    read = {
        k: {
            **v,
            "blocks": [
                {**b0, "content": "x"} if i == 0 else b0 for i, b0 in enumerate(v["blocks"])
            ],
        }
        for k, v in empty.items()
    }
    assert base.run_has_content(run, read)
    names = {m.name for m in base.applicable(registry.METRICS, b, run, truth, read)}
...
def test_a_truth_silent_about_a_trait_is_not_counted_as_marked():
    ...
    truth = {
        i: {
            ...
            "meta": {},
            "blocks": [
                {"block_id": 0, "box": [0, 0, 10, 10], "label": "text", "score": 1.0, "order": 0}
            ],
        }
        for i in range(3)
    }
...
def test_an_error_count_carries_the_pairs_it_was_counted_over():
    ...
    model = {
        0: {
            **truth[0],
            "blocks": [
                {**b, "label": "abstract" if b["block_id"] < 3 else "text"}
                for b in truth[0]["blocks"]
            ],
        }
    }
    rec = contour.ContourMetric().record(contour.compare_pages(truth, model), "b", "m")
...
def test_the_five_artefact_outcomes_account_for_every_object():
    ...
    assert total == counts[0][1], (
        f"the five outcomes sum to {total} of {counts[0][1]} objects -- they are not a partition, and each would still read as a plausible share while the set means nothing"
    )
...
def test_every_scalar_that_reaches_the_document_declares_its_direction():
    ...
    assert len(files) > 20, (
        f"only {len(files)} result files under results/ -- this check is measuring nothing, and a scalar it never sees declares nothing"
    )
    for f in files:
    ...
    assert names - measured, (
        "every registered metric's scalars are already in results/ -- either a metric stopped emitting, or this loop stopped running, and the unmeasured half of the registry is exactly what it is here for"
    )
    ...
    assert not undeclared, (
        f"{len(undeclared)} scalars reach METRICS.md with no Spec: {undeclared}. Declare each in its metric's `scalars`"
    )
...
def test_every_scalar_is_placed_where_its_spec_says(slovar, slovar_run):
    ...
    assert declared - seen == unplaced, (
        sorted(declared - seen - unplaced),
