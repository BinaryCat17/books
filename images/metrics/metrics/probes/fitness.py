...
def _edit(M, fn):
    return {i: {**p, "blocks": [b for b in map(fn, p["blocks"]) if b]} for i, p in M.items()}
                    ...
                    bl.append(
                        {
                            **b,
                            "block_id": n,
                            "box": [x, y, min(x + step - 1, x1), min(y + step - 1, y1)],
                        }
                    )
                    n += 1
        out[i] = {**p, "blocks": bl}
...
def probes(bench, run) -> list:
    ...
    def _dump(M, d):
        for i, p in M.items():
            with open(os.path.join(d, f"{i:04d}.json"), "w", encoding="utf-8") as f:
            ...
            for i, p in M.items():
                ...
                if j.any():
                    ...
                    for a, b in zip(np.flatnonzero(d == 1), np.flatnonzero(d == -1), strict=True):
                        add.append(
                            {
                                "block_id": 900000 + len(add),
                                "box": [int(a), 0, int(b - 1), im.shape[0] - 1],
                                "label": "text",
                                "score": 1.0,
                                "order": 0,
                                "content": None,
    ...
    return [
        ...
        for n, e, f in (
            ...
            (
                "artefact boxes cut in half",
                "and no ink is lost by it",
                lambda: (
                    None
                    if not base["objects"]
                    else halved()["object_ink_in_boxes"] >= base["object_ink_in_boxes"]
                ),
            ),
            (
                "nesting",
                "no more cut as one picture than intact",
                lambda: None if not base["objects"] else base["in_one_box"] <= base["intact"],
            ),
            (
                "no boxes at all",
                "zero ink under boxes",
                lambda: R(_edit(M0, lambda b: None))["ink_under_boxes"] == 0,
            ),
            (
                "boxes moved off the top-left corner",
                "zero ink under boxes",
                lambda: R(_edit(M0, _offpage))["ink_under_boxes"] == 0,
            ),
            (
                "boxes shrunk by half",
                "less object ink preserved",
                lambda: (
                    None
                    if not base["objects"]
                    else R(_edit(M0, lambda b: _scale(b, 0.5)))["object_ink_in_boxes"]
                    < base["object_ink_in_boxes"]
                ),
            ),
            (
                "artefacts called text",
                "zero ink under artefacts",
            ...
            (
                ...
                lambda: (
                    None
                    if not base["objects"]
                    else R(_edit(M0, lambda b: None if art(b) else b))["intact"] < base["intact"]
                ),
            ),
            (
            ...
            (
                ...
                lambda: (
                    ...
                    else _at("MID", 0.5, lambda: R2()["ink_junk"]) >= base["ink_junk"]
                ),
            ),
            (
                ...
                lambda: (
                    None
                    if not base["dark_columns"]
                    else _at("RULE_RUN", 1.01, lambda: R2()["ink_junk"]) >= base["ink_junk"]
                ),
            ),
            (
            ...
            (
                ...
                lambda: (
                    R(_edit(M0, lambda b: None))["ink_junk"] == base["ink_junk"]
            ...
            (
                ...
                lambda: (
                    ...
                    if not base["ink_junk"]
                        ...
                        lambda r: (
                            r["ink_under_boxes"] > base["ink_under_boxes"]
            ...
            (
                ...
                "ink leaves as text where it left as a picture, and the artefacts do not move",
                lambda: (
                    None
                    if not any(
                        (
                        ...
                        lambda r: (
                            r["ink_as_text"] > base["ink_as_text"]
            ...
            (
                ...
                "the area guard does not move ONE PIXEL, and the ink does not either -- the median box and the bill are what see it",
                lambda: (
                    lambda r: (
                        r["boxes_area"] == base["boxes_area"]
                        and r["ink_under_boxes"] == base["ink_under_boxes"]
                        and (r["median_box_area"] < base["median_box_area"] / 10)
                        and (r["box_count"] > base["box_count"] * 10)
                    )
                )(R(_traced(M0))),
            ),
            (
            ...
            (
                ...
                lambda: (
                    ...
                    if not base["objects"]
                        ...
                        lambda r: (
                            None
                            if not r["boxes_with_many_objects"]
                            else r["intact"] >= base["intact"]
                            and r["object_ink_in_boxes"] >= base["object_ink_in_boxes"]
                            and (r["arrived_with_company"] > base["arrived_with_company"]),
                            f"intact {base['intact']} -> {r['intact']}, torn {base['torn']} -> {r['torn']}, with company {base['arrived_with_company']} -> {r['arrived_with_company']}",
                        )
                    )(R(_merge(M0, pol)))
                ),
            ),
            ...
            (
                ...
                lambda: (
                    _at("INK", 0, lambda: ink.measure(pdf, detect_dir, pol=pol)["ink_total"]) == 0
                    and _at(
                        "INK",
                        256,
                        lambda: (lambda r: r["ink_total"] == r["sheet_area"])(
            ...
            (
                ...
                lambda: (
                    ...
                    if not base["objects"]
                        ...
                        "WHOLE",
                        0.0,
                        lambda: (lambda r: r["intact"] == r["objects"])(
            ...
            (
                ...
                lambda: (
                    None
                    if not base["almost_intact"] + base["bitten"]
                    else _at(
                        "ALMOST",
                        ink.WHOLE,
            ...
            (
                ...
                lambda: (
                    ...
                    if not base["dark_columns"]
                        ...
                        "GUTTER",
                        0.0,
                        lambda: ink.measure(pdf, detect_dir, truth_dir)["ink_in_dark_columns"],
                    )
                    >= base["ink_in_dark_columns"]
                    and _at(
                        "GUTTER",
                        1.0,
                        lambda: ink.measure(pdf, detect_dir, truth_dir)["dark_columns"],
                    )
                    < base["dark_columns"]
                ),
            ),
            (
                ...
                lambda: (
                    None
                    if base["ink_total"] <= base["ink_under_boxes"]
                    else (
                        lambda lost: (
                            _at(
                                "EDGE",
            ...
            (
                ...
                lambda: (
                    ...
                    if not base["objects"]
                        ...
                        f"object ink {base['object_ink']} -> {moved()['object_ink']}, preserved {base['object_ink_in_boxes'] / max(1, base['object_ink']) * 100:.1f}% -> {moved()['object_ink_in_boxes'] / max(1, moved()['object_ink']) * 100:.1f}%",
                    )
                ),
            ),
            (
                ...
                lambda: (
                    ...
                    if not base["objects"]
                        ...
                        lambda r: (
                            r["objects"] == 0
                            and r["empty_objects"] == base["objects"] + base["empty_objects"]
                        )
