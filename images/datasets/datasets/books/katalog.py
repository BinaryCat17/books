...
ABOUT = 'parts catalogue: a page with no prose, long tables running over many pages, a repeated header, "Continuation of Table", a footnote under the table'
...
def _head(pg, t, page_no, right="SHAFT FITS"):
    _running_head(pg, t, MARGIN, PW - MARGIN, TOP - 14, "PART CATALOGUE", right, page_no)


def _cat_table(pg, t, x, y, n_cols, rows, colw=52.0, step=9.6, size=5.8, numbered=False):
...
def c_kat_continued(doc, rng):
    ...
    t = []
    _head(pg, t, 215)
    w = _put(pg, MARGIN + 8, TOP + 30, "Continuation of Table 7", 6.6, sheet_w=PW)
    t.append((MARGIN + 6, TOP + 30 - 6.6, MARGIN + 8 + w + 2, TOP + 33, "paragraph_title"))
...
def c_kat_row_numbers(doc, rng):
    ...
    t = []
    _head(pg, t, 216)
    _cat_table(pg, t, MARGIN + 30, TOP + 34, 6, 60, colw=62.0, step=10.4, numbered=True)
    return (pg, t)
...
def c_kat_footnote_under(doc, rng):
    ...
    for k in range(2):
        ln = f"* Values for grade {k + 6} are given in Table {12 + k}; see also note on p. {180 + k * 3}."
        _put(pg, MARGIN + 8, y + 26 + k * 10, ln, 5.4, sheet_w=PW)
        t.append(
            (
                MARGIN + 6,
                y + 26 + k * 10 - 5.4,
                MARGIN + 8 + _text_w(ln, 5.4) + 2,
                y + 28 + k * 10,
                "footnote",
            )
        )
...
def c_kat_two_stacked(doc, rng):
    ...
    y = TOP + 34
    for k in range(2):
        w = _put(pg, MARGIN + 8, y, f"Table {21 + k}.  Bore tolerances", 6.6, sheet_w=PW)
        t.append((MARGIN + 6, y - 6.6, MARGIN + 8 + w + 2, y + 3, "paragraph_title"))
        _say(t, f"Table {21 + k}.  Bore tolerances")
        y = _cat_table(pg, t, MARGIN + 8, y + 26, 6, 26, colw=62.0, step=10.2) + 30
    return (pg, t)
...
def c_kat_spread_continue(doc, rng):
    ...
    t = []
    _running_head(pg, t, MARGIN, 2 * PW - MARGIN, TOP - 14, "PART CATALOGUE", "SHAFT FITS", 223)
