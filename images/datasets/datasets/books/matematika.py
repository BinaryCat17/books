...
COLW = PW - 2 * MARGIN
ABOUT = "mathematics course: numbered display formulas, matrices and determinants, systems in a brace, theorems; sets `display_formula` against `table`"
FORMULAS = (
    "s = (a + b) / 2c",
    "R = 4 s^2 / (h - k)",
    "y = sum a_i x^i + b",
...
def c_mat_matrix_vs_table(doc, rng):
    ...
    _table(
        ...
        MARGIN + 230,
        150,
        _grid(MARGIN + 230, 4, colw=42.0, gap=4.0),
        5,
        size=6.2,
        colw=42.0,
        step=10.5,
...
def c_mat_system(doc, rng):
    ...
    x, y = (MARGIN + 70, 190.0)
    lines = ("a11 x1 + a12 x2 = b1", "a21 x1 + a22 x2 = b2", "a31 x1 + a32 x2 = b3")
...
def c_mat_theorem(doc, rng):
    ...
    for title in ("THEOREM 3.1.", "PROOF."):
        ...
        t.append((MARGIN + 22, y + 18 - 7, MARGIN + 24 + w + 2, y + 20, "paragraph_title"))
        _say(t, title)
...
def c_mat_numbered_list(doc, rng):
    ...
    for k in range(9):
        ...
        t.append((MARGIN + 14, y - 6.6, MARGIN + 34 + _text_w(ln, 6.6) + 2, y + 2, "text"))
        _say(t, f"{k + 1}. {ln}")
        y += 13.0
    _flow(pg, t, MARGIN, y + 12, BOT_Y, PROSE_EN, w=COLW)
    return (pg, t)
...
def c_mat_tall_fraction(doc, rng):
    ...
    for k in range(4):
        ...
        _put(pg, x + (wt - _text_w(bot, 7.6, "M")) / 2, y + 16, bot, 7.6, font="M", sheet_w=PW)
