...
ABOUT = "collection of articles: title and authors, abstract, boxed insert, text wrap, a caption beside the figure, footnotes, references"
...
def c_zh_photo_and_table(doc, rng):
    ...
    _table(
        ...
        COL_X[0] + 4,
        400,
        _grid(COL_X[0] + 4, 3, colw=68.0, gap=6.0),
        22,
        size=5.8,
        colw=68.0,
        step=9.6,
...
def c_zh_references(doc, rng):
    ...
    t.append((MARGIN + 8, TOP + 2, MARGIN + 10 + w + 2, TOP + 15, "paragraph_title"))
    _say(t, "REFERENCES")
...
def c_zh_pull_quote(doc, rng):
    ...
    body = _fill(
        pg,
        r,
        "A simple indicating jig is then made up and the carriage is placed at the mid-point on the bed ways. ",
        8.4,
    )
...
def c_zh_mixed(doc, rng):
    pg = _sheet(doc)
    t = []
    _running_head(pg, t, MARGIN, PW - MARGIN, TOP - 16, "MACHINE TOOLS", "SEC. 26", 70)
    ...
    _table(
        ...
        COL_X[1] + 4,
        320,
        _grid(COL_X[1] + 4, 3, colw=68.0, gap=6.0),
        18,
        size=5.8,
        colw=68.0,
        step=9.6,
