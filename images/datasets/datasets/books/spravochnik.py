...
SHEET = (W, H)
ABOUT = "English technical handbook of the fifties: dense two-column setting, tables without rules, drawings, spreads and rotations"
...
def c_table_across_gutter(doc, rng):
    ...
    _table(pg, t, MARGIN + 6, 250, [(f"Col {i}", MARGIN + 6 + i * 105) for i in range(4)], 7)
    for x in COL_X:
...
def c_three_column_table(doc, rng):
    ...
    for i, x in enumerate((MARGIN + 6, MARGIN + 158, MARGIN + 310)):
        ...
        _say(t, f"TABLE {i + 1}")
        _table(pg, t, x, 236, [(f"Col {i + 1}", x), ("inc.", x + 46)], 9, colw=46)
...
def c_formula_next_to_table(doc, rng):
    ...
    _table(
        ...
        MARGIN + 6,
        260,
        [("d, mm", MARGIN + 6), ("R, MPa", MARGIN + 90), ("K", MARGIN + 174)],
        6,
    )
    _flow(pg, t, MARGIN, 380, BOT, PROSE_EN, w=2 * COLW + GUT)
    return (pg, t)
...
def c_contents_dots(doc, rng):
    ...
    while y < BOT:
        ...
        pg.insert_text((MARGIN + 6, y), name + " " + "." * 46, fontname="F", fontsize=6.6)
        pg.insert_text((PW - MARGIN - 26, y), str(100 + int(y) % 400), fontname="F", fontsize=6.6)
...
def c_full_page_table(doc, rng):
    pg = _page(doc)
    t = []
    _table(pg, t, MARGIN + 6, TOP + 10, [(f"Col {i}", MARGIN + 6 + i * 88) for i in range(5)], 66)
    return (pg, t)
...
def c_russian(doc, rng):
    ...
    _table(
        ...
        COL_X[0] + 6,
        y + 14,
        list(zip(narrow, (COL_X[0] + 6, COL_X[0] + 76, COL_X[0] + 146), strict=False)),
        6,
    )
...
def _half(pg, t, x0, rng, kind):
    ...
    if kind == "table":
        ...
        _table(
            ...
            cx[0] + 6,
            y + 14,
            [("Tool Room", cx[0] + 6), ('12" inc.', cx[0] + 76), ('20" inc.', cx[0] + 146)],
            6,
        )
...
def c_spread_rotated(doc, rng):
    ...
    _table(pg, t, MARGIN + 6, 180, [(f"Col {i}", MARGIN + 6 + i * 118) for i in range(8)], 34)
...
def c_table_tall_narrow(doc, rng):
    pg = _page(doc)
    t = []
    _table(pg, t, COL_X[0] + 6, TOP + 10, _grid(COL_X[0] + 6, 2, colw=90), 58, colw=90)
...
def c_table_ruled(doc, rng):
    ...
    _table(pg, t, MARGIN + 6, 270, _grid(MARGIN + 6, 5, colw=76), 14, colw=76, ruled=True)
...
def c_table_split_b(doc, rng):
    pg = _page(doc)
    t = []
    _table(pg, t, MARGIN + 6, TOP + 6, _grid(MARGIN + 6, 5, colw=76), 22, colw=76)
...
def c_russian_table_wide(doc, rng):
    ...
    _table(
        ...
        list(
            zip(
                HEADS_RU,
                (MARGIN + 6, MARGIN + 96, MARGIN + 186, MARGIN + 276, MARGIN + 356),
                strict=False,
            )
        ),
        16,
...
def c_spread_table_wide(doc, rng):
    ...
    _table(pg, t, MARGIN + 6, 290, _grid(MARGIN + 6, 11, colw=76), 20, colw=76)
    for x0 in (0.0, PW):
...
def c_table_two_stacked(doc, rng):
    ...
    y = 176
    for k in range(2):
        pg.insert_text((MARGIN + 6, y), f"TABLE {k + 4}.  Shaft fits", fontname="F", fontsize=7.0)
        t.append((MARGIN + 3, y - 8, MARGIN + 3 + 96, y + 4, "paragraph_title"))
        _say(t, f"TABLE {k + 4}.  Shaft fits")
        y = _table(pg, t, MARGIN + 6, y + 22, _grid(MARGIN + 6, 5, colw=84), 11, colw=84) + 26
...
def c_table_dense_no_rules(doc, rng):
    ...
    _table(
        ...
        MARGIN + 6,
        158,
        _grid(MARGIN + 6, 9, colw=44, gap=2.0),
        62,
        colw=44,
        step=7.4,
        size=5.6,
...
def c_figure_and_table(doc, rng):
    ...
    t = []
    _flow(pg, t, MARGIN, TOP, 150, PROSE_EN, w=2 * COLW + GUT)
    _figure(pg, t, MARGIN + 6, 178, COLW - 6, 200, "Fig. 44  Tailstock")
    _table(
        pg,
        t,
        MARGIN + COLW + GUT + 10,
        178,
        _grid(MARGIN + COLW + GUT + 10, 2, colw=76),
        20,
        colw=76,
    )
    _flow(pg, t, MARGIN, 420, BOT, PROSE_EN, w=2 * COLW + GUT)
...
def c_marginalia(doc, rng):
    ...
    for _k, y0 in enumerate((TOP + 20, 260, 470, 640)):
        ...
        body = _fill(pg, r, "Note. See Sec. 26 for the tolerance grades used here. ", 5.6)
...
def c_footnotes_rule(doc, rng):
    ...
    pg.draw_line(
        _rect(MARGIN, 590, MARGIN + 120, 590).tl,
    ...
    for k in range(3):
        ...
        body = _fill(pg, r, f"{k + 1} Trans. A.S.M.E., vol. 61, p. {120 + k * 7}. ", 5.6)
        t.append((MARGIN, 598 + k * 26, MARGIN + 2 * COLW + GUT, 620 + k * 26, "footnote"))
        _say(t, body)
...
def c_rotated_single_table(doc, rng):
    ...
    _table(pg, t, MARGIN + 6, 170, _grid(MARGIN + 6, 8, colw=50), 36, colw=50, step=8.4)
    return (pg, t)
...
def c_chart_pair(doc, rng):
    ...
    _chart(pg, t, MARGIN + COLW + GUT + 24, 200, COLW - 40, 150, "Fig. 62  Toughness")
    _flow(pg, t, MARGIN, 420, BOT, PROSE_EN, w=2 * COLW + GUT)
    return (pg, t)
...
ROTATE = {"spread_rotated": 90, "spread_rotated_figure": 90, "rotated_single_table": 90}
SPREADS = {"spread", "spread_table_wide", "spread_rotated", "spread_rotated_figure"}
