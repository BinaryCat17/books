...
def query(entry: dict) -> str:
    q = [f"gpu_name={entry.get('gpu') or 'RTX_4090'}", "num_gpus=1", "rentable=true", "verified=true",
...
class Vast:
    ...
    def start(self, model: str, entry: dict, key: str) -> dict:
        ...
        offer = offers[0]
        port = entry["port"]
        env = " ".join([f"-p {port}:{port}", f"-e BOOKSMITH_SERVE_KEY={key}", f"-e PORT={port}"]
    ...
    def alive(self, handle: str) -> bool:
        try:
            return any(str(i.get("id")) == str(handle) for i in self.v.show_instances())
        except Exception:
            return True

    def stop(self, handle: str) -> None:
        for pause in (4, 8, 16, 32, 60):
            try:
            ...
            if not self.alive(handle):
                return
        log(f"COULD NOT DESTROY {handle}: still billing; destroy it by hand")
    ...
    def rate(self, entry: dict) -> float:
        return float(entry.get("max_dph", 0.6))
