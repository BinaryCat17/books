...
Environment: `MODEL_NAME`, `VL_MODEL_DIR`, `PORT` (vLLM's), `BOOKSMITH_PORT`
(the shim's), `BOOKSMITH_UPSTREAM` (a vLLM already up, instead of raising one),
`BOOKSMITH_SERVE_KEY`. Routes: `schema/openapi/model.yaml`. Tests: `pytest`.
