...
instead of raising one), `BOOKSMITH_LOG_DIR` (writable), `BOOKSMITH_SERVE_KEY`,
`BOOKSMITH_IDLE_S` (a dead-man: with no request that long the container exits,
and on vast destroys its instance). Routes: `schema/openapi/model.yaml`. Tests: `pytest`.
